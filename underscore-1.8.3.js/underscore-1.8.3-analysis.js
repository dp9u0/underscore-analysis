...
(function() {
    ...
    // 用来将 全局的 _ 变量还原为 previousUnderscore
    ...
    var _ = function(obj) {
        ...
        if (!(this instanceof _))
            return new _(obj); // this._wrapped = obj;
    ...
    // 其实就是根据 value 类型,返回一个callback
    // No value => _.identity()
    // Function => function(n) { return n * 2; }
    // Object => _.matcher({firstName: 'Chelsea'});
    // Anything else => _.property('firstName');
    ...
    var createAssigner = function(keysFunc, undefinedOnly) {
        ...
        // 返回的函数参数个数 >= 1
        // 将第二个开始的对象参数的可枚举属性复制给第一个参数
        // 继承哪些键值对根据 
        // keysFunc(_.key : ownProperty or _.allKeys : 原型链上的所有属性)
        // undefinedOnly: 是否只覆盖未定义的属性
        return function(obj) {
    ...
    // use in `_.create`
    // 创建一个空对象 并且指定 __proto__ 为 prototype
    ...
    // 包括字符串、函数等
    // 判断依据 : 一个 object 是否有 length 属性 并且length < MAX_ARRAY_INDEX
    ...
    // 遍历数组或者对象的每个元素
    // 第一个参数为数组（包括类数组）或者对象
    // 第二个参数为迭代方法，对数组或者对象每个元素都执行该方法 第二个参数只能必须是方法
    ...
    // notice: iteratee 必须是function
    _.each = _.forEach = function(obj, iteratee, context) {
        // 觉得这里最好提前判断一下 iteratee 是否是function
        // 否则下面调用的时候才会会直接报错
    ...
    */
    ...
    _.memoize = function(func, hasher) {
        ...
        var memoize = function() {
            ...
            // TODO:为什么不适用 arguments.concat 作为 address 呢?
            var address = '' + (hasher ? hasher.apply(this, arguments) : arguments[0]);
